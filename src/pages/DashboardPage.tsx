...
function DashboardPage() {
  ...
  const metrics = [
    { label: 'Projects shipped', value: '04', detail: 'Across four disciplines' },
  ...
  return (
    <main className="mx-auto max-w-6xl px-6 py-16 sm:py-20">
        ...
        <div className="flex flex-col justify-between gap-6 sm:flex-row sm:items-end">
          <div>
            ...
            <p className="mt-4 max-w-2xl text-lg leading-8 text-slate-600 dark:text-slate-300">
              A quick view of the systems, skills, and delivery experience behind this portfolio.
            </p>
          </div>
      ...
      <section className="mt-10 grid gap-6 lg:grid-cols-[1.15fr_0.85fr]" aria-label="Portfolio overview">
        ...
        <article className="rounded-2xl border border-slate-200 bg-slate-200 p-6 shadow-sm dark:border-slate-700 dark:bg-slate-900 sm:p-8">
          ...
          <div className="mt-8 space-y-5">
            {['Full-stack systems', 'Cloud-native delivery', 'AI-powered products'].map((focus, index) => (
              <div key={focus} className="flex items-center gap-4">
                ...
                <div className="flex-1">
                  <p className="font-medium text-slate-950 dark:text-white">{focus}</p>
                  <div className="mt-2 h-1.5 rounded-full bg-slate-300 dark:bg-slate-700">
                    <div className="h-1.5 rounded-full bg-blue-500 dark:bg-blue-400" style={{ width: `${88 - index * 12}%` }} />
                  </div>
                </div>
              </div>
            ))}
      ...
      <section className="mt-10" aria-labelledby="dashboard-projects-heading">
        <div className="flex items-end justify-between gap-4">
          <div>
            ...
            <h2 id="dashboard-projects-heading" className="mt-2 text-2xl font-semibold tracking-tight text-slate-950 dark:text-white">
              Featured systems
            </h2>
          </div>
        ...
        <div className="mt-6 grid gap-4 md:grid-cols-2">
          {projects.slice(0, 4).map((project) => (
            <article key={project.id} className="flex items-center gap-4 rounded-2xl border border-slate-200 bg-slate-200 p-5 shadow-sm dark:border-slate-700 dark:bg-slate-900">
              <span className="h-3 w-3 shrink-0 rounded-full bg-emerald-500 ring-4 ring-emerald-100 dark:ring-emerald-950" />
              <div className="min-w-0">
                <h3 className="truncate font-semibold text-slate-950 dark:text-white">{project.title}</h3>
                <p className="mt-1 text-sm text-slate-500 dark:text-slate-400">{project.category}</p>
              </div>
              <span aria-hidden="true" className="ml-auto text-xl text-slate-400">→</span>
            </article>
          ))}
        </div>
      </section>
